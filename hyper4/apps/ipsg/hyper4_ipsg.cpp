...
#include "hyper4_ipsg.h"
#include "../../hyper4_match.h"
#include "../../hyper4_action.h"

void
hyper4_ipsg::activate() {

}

void hyper4_ipsg::install(uint32_t src_ip,
                          uint16_t iport) {


    uint8_t stage_id = start_stage;
    {
        hyper4_intrinsic_match match(controller->get_p4info(),
                                  this->program_id,
                                  this->start_stage);

        match.match_ingress_port(iport);
